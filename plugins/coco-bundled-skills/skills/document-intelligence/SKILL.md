...
"description": "Document intelligence over files, PDFs, images, and stage documents with Snowflake Cortex AI functions — extract fields, parse/OCR text, classify by type, and visually analyze charts/diagrams, for one file or a one-time batch. Also fine-tunes arctic-extract. Use when extracting structured fields from PDFs/forms/invoices, parsing or OCR-ing a document, classifying/triaging documents by type, analyzing a chart/blueprint/drawing, or any one-off AI task over files already on a stage. Triggers: extract from PDF, extract fields, structured extraction, parse document, get text from PDF, OCR, classify/categorize/triage documents, document type, invoice, contract, form, blueprint, drawing, diagram, chart, files on stage, AI_EXTRACT, AI_PARSE_DOCUMENT, AI_CLASSIFY, AI_COMPLETE vision, fine-tune, train arctic-extract, FINETUNE. To build a document pipeline — chaining AI functions into one flow, or keeping outputs fresh as new files land (stream → task → dynamic tables) — use ai-functions-pipeline-builder instead."
---
