...
"description": "Build Snowflake-native document and file pipelines with Cortex AI functions. Turns a request into an incremental pipeline (stream → task → INCREMENTAL dynamic tables) that ingests files from a stage and keeps outputs fresh as new files land. Routes to AI_EXTRACT (structured fields), AI_PARSE_DOCUMENT (full text/OCR), AI_COMPLETE (visual/chart/diagram analysis), AI_CLASSIFY (categorize/triage). Use when building a document/file processing or incremental stage-ingestion pipeline, or an enterprise-search / corpus-intelligence / structured-extraction / customer-360 pipeline. Triggers: document pipeline, incremental pipeline, ingestion pipeline, keep outputs fresh, process new files as they land, stream and task, dynamic tables, enterprise search, customer 360. For a one-off task over files you already have (single file or one-time batch) use document-intelligence. For standalone AI functions over already-tabular text or image rows with no file, stage, or document, defer to cortex-ai-function-studio."
---
