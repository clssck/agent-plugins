...
| Tasks | [`data-engineering/snowflake-tasks/SKILL.md`](../../data-engineering/snowflake-tasks/SKILL.md) | **Active (history-driven)** | Alert-troubleshoot now delegates directly to this skill. Troubleshooting is primarily via "Querying Task Run History" (`INFORMATION_SCHEMA.TASK_HISTORY`) and failure diagnostics (state/error fields), even though there is no separate standalone `tasks-troubleshoot` sub-skill file. |
...
| Tasks | `snow.executable.type = 'TASK'` OR `INFORMATION_SCHEMA.TASK_HISTORY` OR `SNOWFLAKE.ACCOUNT_USAGE.TASK_HISTORY` OR template id `TASKS_*` | Route to [`snowflake-tasks/SKILL.md`](../../data-engineering/snowflake-tasks/SKILL.md) (active, history-driven troubleshooting). |
| Data Quality | `SNOWFLAKE.LOCAL.DATA_QUALITY_MONITORING_RESULTS` / `SNOWFLAKE.LOCAL.DATA_QUALITY_MONITORING_EXPECTATION_STATUS` OR template id `DQ_*` | Route to [`data-quality/SKILL.md`](../../data-governance/data-quality/SKILL.md) (active). |
