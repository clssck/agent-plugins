...
"description": "Use for ALL requests related to Snowflake Data Clean Rooms (DCR). Covers browsing/joining collaborations and invitations, registering and sharing tables, linking and unlinking data offerings, running analysis and activation, managing and authoring templates, DCR RBAC/privilege setup, tearing down collaborations, and custom registries via the DCR Collaboration API. Triggers: clean room, cleanroom, DCR, collaboration(s), view/list collaborations, join/review collaboration, invitation, data offering, template, register, share table, run analysis, run activation, audience overlap, export segment, create collaboration, add/remove/approve/reject template, auto-approval, link/unlink data offering, share data with runner, revoke data access, tear down, leave/drop/delete collaboration, DCR roles, DCR privileges, grant collaboration privileges, create/list/manage registries, custom registry, template spec, Jinja template, incrementality measurement, multi-touch attribution, convert or migrate PnC template."
allowed-tools:
...
Intent Detection
  ...
  +---> MANAGE REGISTRIES --> Load manage-registries/SKILL.md
...
2. **Route to Sub-Skill** — Detect intent from user request and **use the `read` tool** to load the matching sub-skill:
   ...
   **MANAGE REGISTRIES** — "create registry", "custom registry", "new registry", "list registries", "view registries", "manage registries"
   → Load `manage-registries/SKILL.md`
...
| Create and list custom registries | `manage-registries/SKILL.md` | Yes (confirm before create) |
...
- **Manage Registries**: Confirm registry name + type before creating (a separate confirmation for every registry)
...
- `manage-registries/SKILL.md` - Create and list custom registries
