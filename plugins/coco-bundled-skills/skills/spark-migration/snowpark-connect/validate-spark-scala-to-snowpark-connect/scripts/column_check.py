#!/usr/bin/env python3
"""Scala analyzer gate: column_refs + write_helpers coverage vs schemas/.

Replaces the manual LLM body-scan. Run after ``datagen.py`` (unchanged
PySpark script) and before recording ``synth_deep``. Prefers ``schemas/`` as
SoT; regenerates the analysis shim when present so checks cannot drift.
...
def _refresh_analysis_from_schemas(conv_root: Path) -> Path | None:
    """If schemas/manifest.json exists, regenerate analysis.json and return its path."""
    schemas_manifest = conv_root / "Validation" / "shared" / "schemas" / "manifest.json"
    if not schemas_manifest.is_file():
        return None
    sm_path = Path(__file__).resolve().parent / "schema_mine.py"
    spec = importlib.util.spec_from_file_location("scos_schema_mine_cc", sm_path)
    if spec is None or spec.loader is None:
        return None
    mod = importlib.util.module_from_spec(spec)
    try:
        spec.loader.exec_module(mod)
        mod.schemas_to_analysis_shim(conv_root)
    except SystemExit:
        raise
    except Exception as exc:
        print(f"[column_check.py] WARNING: schemas→analysis shim failed: {exc}",
              file=sys.stderr)
        return None
    return conv_root / "Validation" / "shared" / "analysis.json"
...
def _analysis_sink_catalog(analysis: dict) -> dict[str, dict]:
    """Global sink catalog — merges ``sinks`` + ``external_sinks`` (synthesizer key)."""
    catalog: dict[str, dict] = {}
    for coll in (analysis.get("sinks") or [], analysis.get("external_sinks") or []):
        for sink in coll:
...
def _ep_sink_items(ep: dict) -> list:
    """Per-entrypoint sink refs — merges ``sinks`` + ``external_sinks``."""
    merged: list = []
...
def check_columns(
    ...
    *,
    ...
    """Verify column_refs and write_helpers against declared schemas (shim or analysis)."""
    problems: list[str] = []
    ...
    for ep in analysis.get("entrypoints") or []:
        ...
        # Normalise col_refs before checking:
        # - Drop wildcard selectors like "latest.*" (can't validate statically).
        # - Strip table-alias qualifiers like "lcl.gst" → "gst" so that columns
        #   declared in source schemas without aliases still satisfy the check.
        col_refs = []
        for c in col_refs_raw:
            if c.endswith(".*"):
                continue  # wildcard — skip
            if "." in c:
                c = c.split(".")[-1]  # strip qualifier prefix
            col_refs.append(c)
        col_refs = sorted(set(col_refs))
        ...
        if write_helpers:
            ...
            for helper in sorted(write_helpers):
                ...
                if hlow not in sink_ids and hlow not in sink_names:
                    problems.append(
                        "%s: write_helper '%s' has no matching sinks[]/external_sinks[] entry"
                        % (ep_id, helper)
                    )
    return problems
...
def main(argv: list[str] | None = None) -> int:
    ap = argparse.ArgumentParser(
        ...
        description=(
            "Scala body-scan gate: ast_facts column_refs vs schemas/ "
            "(analysis.json shim)."
        ),
    )
    ...
    if args.conv_root:
        ...
        # Prefer schemas/ SoT: refresh shim before reading analysis.json.
        _refresh_analysis_from_schemas(conv_root)
