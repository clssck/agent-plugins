...
**only** mode: `analyze_java.py` never calls `SNOWFLAKE.CORTEX.COMPLETE`.
...
2. For each `needs_adjudication` row on that file, examine the code at `lines` in
   ...
   - **Real incompatibility** — it needs a code change. Emit:
     ...
     {"file": "<abs path>", "lines": "<lines>",
      ...
      "fix": "<recommended Java fix, grounded in references/fix-rules.md>"}
     ```
...
- **Do NOT inflate plain Java Spark.** Ordinary Dataset/DataFrame transforms —
...
- **HARD FLOOR — always confirm (final_risk ≥ 0.9), even from your own knowledge:**
  ...
  `references/java/rdd-conversion.md`; unsupported ecosystem libs (GraphFrames,
