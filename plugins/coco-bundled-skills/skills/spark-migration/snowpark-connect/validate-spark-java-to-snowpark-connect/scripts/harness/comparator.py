...
This module re-exports that file so the Java ``compare_trial.py`` and summary
auto-compare never drift from a local fork. Identical to the Scala validator's
copy of this shim — both sit at the same relative depth
(``validate-spark-{java,scala}-to-snowpark-connect/scripts/harness/``) under
``snowpark-connect/``, so the ``parents[3]`` resolution below is unchanged.
"""
