"""Tests for scos_state.py batch-orchestration additions (Java variant):
...
def _make_orig_source(tmp_path: Path) -> Path:
    ...
    orig.mkdir()
    (orig / "Main.java").write_text("public class Main { public static void main(String[] args) {} }")
    return orig
...
def test_prepare_batches_creates_batches_prepared_json(tmp_path, monkeypatch):
    ...
    conv.mkdir()
    (conv / "Output").mkdir()
    (conv / "Output" / "Main.java").write_text("public class Main { public static void main(String[] args) {} }")
...
def test_prepare_batches_coverage_check_fails_on_missing_ep(tmp_path, monkeypatch):
    ...
    conv.mkdir()
    (conv / "Output").mkdir()
    (conv / "Output" / "Main.java").write_text("")
    eps = [{"id": "ep1"}, {"id": "ep2"}]
    _make_analysis(conv, eps)
...
def test_derive_phase_reads_java_state_json(tmp_path, monkeypatch):
    """batch.py._derive_phase must not crash on Java state.json and must handle terminal phases."""
    pyspark_scripts = (
    ...
    assert derived_init != derived_done, (
        ...
        f"phase_b_done ({derived_done!r}) — Java state.json schema may have drifted"
    )
