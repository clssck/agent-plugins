...
data contract it consumes — `load_validation_run` over the Java validator's
on-disk artifacts — is. This guards the Java-specific adaptations: analysis is
read from `shared/analysis.json` (not a PySpark schemas/manifest split), and the
Java milestones + per-trial `migration_fix_commits` surface correctly.
"""
...
def _make_run(root: Path) -> None:
    ...
    run_index = {
        ...
        "milestones": {"analyzer_survey": {"status": "done"},
                       "workload_built": {"status": "done"},
                       "patches_authored": {"status": "pending"}},
        "entrypoints": [{
            "id": "ep1",
            "source_path": "src/Job.java",
    ...
    (root / "events.jsonl").write_text(
        json.dumps({"ts": "2026-01-01T00:00:00Z", "kind": "milestone_completed", "milestone": "analyzer_survey"}) + "\n")


def test_load_validation_run_java(tmp_path):
    root = tmp_path / "Validation"
    _make_run(root)
    data = rd.load_validation_run(root)

    # analysis comes from shared/analysis.json (Java), not a manifest split
    ...
    # Java milestones present in the milestone model
    names = {m["id"] for m in data.milestones}
    assert {"analyzer_survey", "workload_built", "patches_authored"} <= names
...
def test_milestone_descriptions_are_java():
    # No PySpark-only or Scala-only milestone wording leaked through the vendoring.
    ...
    assert "analyzer_survey" in rd.MILESTONE_ORDER
    assert "analyzer_deep" in rd.MILESTONE_ORDER
