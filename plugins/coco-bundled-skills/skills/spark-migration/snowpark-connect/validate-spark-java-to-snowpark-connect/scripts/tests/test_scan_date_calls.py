...
def test_detects_bare_and_qualified_calls(tmp_path):
    source = tmp_path / "Validation" / "source" / "src" / "Job.java"
    source.parent.mkdir(parents=True, exist_ok=True)
    source.write_text(
        "import org.apache.spark.sql.functions.*;\n"
        "Dataset<Row> a = functions.current_date();\n"
        "Dataset<Row> b = current_date();\n",
        encoding="utf-8",
    )
...
def test_clean_scan_exits_zero(tmp_path):
    ...
    source.parent.mkdir(parents=True, exist_ok=True)
    source.write_text(
        "public class Job { public static void main(String[] args) {} }",
        encoding="utf-8",
    )
