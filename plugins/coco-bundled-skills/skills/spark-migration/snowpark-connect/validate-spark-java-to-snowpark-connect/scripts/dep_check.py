#!/usr/bin/env python3
"""Dependency/classpath validation for Java → Snowpark-Connect workloads.

Mirrors validate-spark-scala-to-snowpark-connect/scripts/dep_check.py, adapted
for Maven-first Java projects.
...
# ---------------------------------------------------------------------------
# Kit defaults — shared Scala test harness (harness-scala/kit/build.sbt)
# ---------------------------------------------------------------------------
...
_SHADED_MARKERS = ("_shaded_", "relocated.", "repackaged.")
_SPARK_INTERNAL_PREFIXES = (
    "org/apache/spark/", "org/apache/hadoop/", "com/google/", "io/netty/",
    "org/slf4j/", "com/fasterxml/", "org/json4s/", "scala/",
)

# ---------------------------------------------------------------------------
# Build-file parsers (Maven-first for Java projects; sbt/Gradle for mixed)
# ---------------------------------------------------------------------------
...
def _parse_pom(content: str) -> dict:
    ...
    Checks ``<spark.version>`` property first; falls back to the dependency
    block immediately after a spark-sql artifactId.
    """
...
def _parse_sbt(content: str) -> dict:
    """Extract Spark/Delta versions from a build.sbt (legacy / mixed projects)."""
...
def _load_output_build_info(output_dir: Path) -> dict:
    ...
    Maven (pom.xml) is tried first as it is the canonical Java build tool.
    """
...
def _find_assembly_jar(output_dir: Path, analysis_jar_path: str) -> Optional[Path]:
    ...
    if target.is_dir():
        # Prefer assembly/uber/fat/shaded jars
        assembly = sorted(
            (j for j in target.rglob("*.jar")
             if any(k in j.name.lower() for k in ("assembly", "uber", "fat", "shaded", "jar-with-dependencies"))
...
def _check_version_alignment(output_dir: Path) -> tuple[list[str], list[str]]:
    ...
    if not build_info:
        warnings.append(
            "dep_check: no pom.xml / build.sbt / build.gradle found in Output/ — "
...
def _check_duplicate_classes(jar_path: Path) -> tuple[list[str], list[str]]:
    """Detect duplicate class entries in the assembly JAR (exact filename match).

    Only raw filename duplicates are flagged as problems. Inner-class normalisation
    was deliberately removed (mirrors Scala dep_check): collapsing ``Foo.class``
    and ``Foo$1.class`` into the same key produced false positives for any class
    with anonymous or named inner classes — a normal pattern in all JVM code.
    """
    problems: list[str] = []
    warnings: list[str] = []
    seen: dict[str, int] = {}
...
def _check_scos_jar(output_dir: Path, conv_root: Path) -> tuple[list[str], list[str]]:
    """Check that the SCOS Java client jar is locatable (mirrors scos_state logic).

    Mirrors the probe sequence in scos_state._locate_scos_jar so the dep check
    agrees with what Phase B would find at runtime.
    """
    ...
    return (
        [
            "dep_check: SCOS Java client jar (snowpark-connect-java-client) not found in "
            "Validation/tests/lib, Output/lib, ~/.m2, or the Coursier cache — "
            "Phase B will fail with ClassNotFoundException. "
            "Place the jar in Output/lib/ before running Phase B, or run "
            "`mvn dependency:resolve` to populate the local Maven cache."
        ],
        [],
    )
...
def run_checks(conv_root: Path, jar_path_override: str = "") -> dict:
    ...
    Safe to import and call directly from scos_state.py prevalidate.
    """
