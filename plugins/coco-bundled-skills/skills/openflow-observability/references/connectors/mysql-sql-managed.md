---
name: openflow-observability-connector-mysql-sql-managed
description: Authoritative property-value catalog for the SQL-managed OPENFLOW_MYSQL_CDC connector. Use when proposing connector.config_set_property action edits or interpreting validation errors that mention specific property values for MySQL CDC.
---

# MySQL CDC -- SQL-Managed Property Catalog

This file is the authoritative reference for valid property values in the SQL-managed `OPENFLOW_MYSQL_CDC` connector definition (the connector created via `CREATE OPENFLOW CONNECTOR ... FROM DEFINITION OPENFLOW_MYSQL_CDC`).

`OPENFLOW_MYSQL_CDC` is a SQL-managed first-class connector. Authorize any mutating action by the live `SHOW` / `DESCRIBE OPENFLOW CONNECTOR` definition match (and the resolved `openflowConnectorId` when page context carries it), not by a static maturity label. MySQL CDC is still maturing; you may optionally note that to the customer in the Confirmation Preview.

**This file is for SQL-managed (gen-2) connector edits only.** The gen-1 MySQL CDC connector, configured through Apache NiFi parameter contexts, uses different property names and a different config shape. For that connector, see [mysql.md](mysql.md).

**Source of truth:** the SQL-managed MySQL connector step classes under `runtime-extensions/runtime-connector-bundles/runtime-mysql-connector-bundle/runtime-mysql-connector/src/main/java/com/snowflake/openflow/runtime/connectors/mysql/`, plus the shared CDC enums under `runtime-connector-commons/runtime-cdc-connector-shared/.../properties/`. The connector validator accepts the value returned by each enum's `DescribedValue.getValue()` method. By default this is the Java enum constant name (`name()`), but enum classes that declare a 3-arg constructor override `getValue()` to return a custom string -- typically the display label, including spaces. **Use the tables below as the authoritative form for each property; do not infer the form from the enum constant name alone.** When in doubt, the validation error message `Value is not one of the allowable values` typically lists the actual valid values in subsequent log lines.

> **Verify before writing.** The driver identity (`MariaDB Connector/J`) and the `jdbc:mariadb://` URL prefix below are documented from the connector source tree, not read live, and the SQL-managed property surface can change between connector versions. Before proposing a `config_set_property` / `config_set_asset` edit that depends on these, confirm them against the connector's live `config.json` snapshot (Connector Config Snapshot in [connector-diagnostics.md](../openflow-sql/connector-diagnostics.md)) or the `DESCRIBE` output. `config_set_asset` already requires the customer's exact JAR basename (the agent never infers it); apply the same "confirm against live state" discipline to the URL form.
...
| `Destination Schema Strategy` | `SOURCE_SCHEMA` / `PREFIX_AND_SOURCE_SCHEMA` / `SOURCE_SCHEMA_AND_SUFFIX` / `CUSTOM` | "Source Schema" / "Custom" |
...
| `Source Database Connection URL` | `STRING_LITERAL` | yes | non-empty; MariaDB JDBC URL form `jdbc:mariadb://<host>:<port>/<db>?<params>` (default port `3306`). When SSL is disabled the URL typically needs `?allowPublicKeyRetrieval=true`. |
| `Source Database Driver` | `ASSET_REFERENCE` | yes | non-empty asset id list. The connector uses the **MariaDB Connector/J** driver (NOT MySQL Connector/J). |

MySQL has **no** `Source Database Publication Name` (that is PostgreSQL-only).
...
| `Included Source Table Pattern` | `STRING_LITERAL` | one of these two | regex pattern, e.g. `orders\..*` |
...
| `Object Identifier Resolution` | `STRING_LITERAL` | yes | `CASE_INSENSITIVE` (default), `CASE_SENSITIVE`. **Immutable after first start.** |
| `Destination Schema Strategy` | `STRING_LITERAL` | yes | `SOURCE_SCHEMA` (default), `PREFIX_AND_SOURCE_SCHEMA`, `SOURCE_SCHEMA_AND_SUFFIX`, `CUSTOM`. **Immutable after first start.** |
...
MySQL exposes 4 `Destination Schema Strategy` values (PostgreSQL's additional `{database}`-bearing strategies do not apply, because MySQL has no separate database tier above the schema):

| `Destination Schema Strategy` value | Companion property required | Meaning |
| --- | --- | --- |
| `SOURCE_SCHEMA` (default) | none | destination schema = source schema |
| `PREFIX_AND_SOURCE_SCHEMA` | `Destination Schema Prefix` | `<prefix>_{schema}` |
| `SOURCE_SCHEMA_AND_SUFFIX` | `Destination Schema Suffix` | `{schema}_<suffix>` |
| `CUSTOM` | `Destination Schema Pattern` | pattern using `{schema}` and `{table}` tokens only |

> **MySQL custom pattern rejects `{database}`.** Only `{schema}` and `{table}` are valid tokens in `Destination Schema Pattern` for MySQL. A pattern containing `{database}` fails validation.
...
| `Starting Binlog Position` | `STRING_LITERAL` | optional | format `<binlog-file>:<position>`, for example `mysql-bin.000042:4`. Set only for break-glass / reinstallation cases to start incremental capture from a known position. |
| `Re-read Tables in State` | `STRING_LITERAL` | optional | comma-separated fully-qualified table names to force a metadata refresh for those tables. |

MySQL has **no** `Replication Slot Name` (that is PostgreSQL-only).
...
2. **Secret type matches the connector's expectation** -- MySQL CDC expects `GENERIC_STRING`, not `PASSWORD`. The `PASSWORD` type packages USERNAME + PASSWORD pairs; MySQL CDC takes the username as a separate property (`Source Database User`) and only consumes the secret value as the password.
