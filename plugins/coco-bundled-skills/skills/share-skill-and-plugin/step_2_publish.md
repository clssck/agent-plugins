...
Runtime modes: [SKILL.md](SKILL.md) § Runtime modes. The CLI handles uploads
itself; the SQL fallback differs by mode only in **§ Upload Files** and by
`<artifact_type>` in CREATE `TYPE` and upload layout.

## Option A: CoCo CLI publish *(try first when shell is available)* 

### Skill — `cortex skill catalog publish`

When `<artifact_type> = skill`:

```bash
# sandbox-only — requires the cortex CLI on PATH
cortex skill catalog publish <artifact_dir>
```

- **When to try:** shell is available (Sandbox or CLI runtime) and
  `<artifact_dir>` is a readable local path. In non-sandbox (SQL-only)
  runtime, skip Option A.
- **No audience collection before CLI** — share options are applied by the CLI
  or deferred to step 3 only on SQL fallback.
- **Surface once:** before invoking, show
  [SKILL.md](SKILL.md) § **Cortex Extension — user-facing one-liner** if
  it hasn't been shown yet during this run.
- **On success:** the CLI prints the resulting share URI. Jump to
  [step 5](step_5_report_result.md); step 3 is **not** required.
- **Fall back to Option B** when the CLI is missing, the command exits
  non-zero, or the intent is `unshare` / `share-resync` (content-only
  re-share) / a rename pivot — **except** the live-version conflict
  below (handle via consent; do **not** fall to Option B for that
  case). Surface the CLI error verbatim before switching paths, but do
  not retry the CLI itself in a loop.
- **Uncommitted live version (CLI `LIVE_VERSION_EXISTS` / 099106)** —
  when the CLI error matches any of these signals, **do not** fall back
  to Option B and **do not** silently ABORT:
  - error text containing `099106`
  - `There is already a live version`
  - `already has an uncommitted live version`
  - structured guidance mentioning `--force-abort-live`

  Required agent behavior:

  1. Reveal to the user that Cortex Extension `<fqn>` (extract from the
     CLI error when present) already has an **uncommitted live version**.
  2. Explain (same framing as the CoCo TUI):
     - This usually means either another publish/share is running against
       the same skill, **or** a previous attempt was interrupted and did
       not clean up.
     - Abort discards that open live version so **this** share can
       continue.
  ...
  4. If the user chooses **Abort live version and continue sharing**:
     - Re-run the **same** `cortex skill catalog publish` invocation with
       the same path/args **plus** `--force-abort-live`.
     - On success, continue as normal Option A success (jump to
       [step 5](step_5_report_result.md)).
     - If it fails again for any **other** reason, surface the error and
       follow existing fallback rules — do **not** loop `--force-abort-live`.
  5. If the user chooses **Cancel sharing**:
     - **Stop.** Do not ABORT. Do not fall back to Option B. Report
       cancelled and go to [step 5](step_5_report_result.md) as
       appropriate.

  Emphasize: do **not** silently abort; do **not** fall through to
  Option B SQL `ADD LIVE VERSION` / `ABORT` for this specific CLI error
  (that would race the same way).
- **Stage limit error** — when the CLI error matches
  [SKILL.md](SKILL.md) § **Stage file size and count limits**, follow that
  section and **stop**; do **not** fall back to Option B.

### Plugin — `cortex plugin publish`
...
Store the result as `<share_choice>` (1 / 2 / 3), `<share_roles>` (list,
empty for choices 1 & 2), and `<discoverable_value>` (TRUE / FALSE):

| Choice | `<share_roles>` | `<discoverable_value>` |
|--------|-----------------|------------------------|
| 1 | `[PUBLIC]` | TRUE |
| 2 | `[PUBLIC]` | FALSE |
| 3 | user-supplied role(s) (uppercase) | TRUE |

These values are carried forward so [step 3](step_3_apply_share_options.md)
does **not** ask the audience question again.

#### Build and run the command

Map the collected values to CLI flags:

| `<share_choice>` | `<share_roles>` | CLI flags to add |
|------------------|-----------------|-----------------|
| 1 (PUBLIC, discoverable) | PUBLIC | `--to-role PUBLIC --discoverable` |
| 2 (PUBLIC, private) | PUBLIC | `--to-role PUBLIC --no-discoverable` |
| 3, **single** role | `[ROLE_NAME]` | `--to-role ROLE_NAME --discoverable` |
| 3, **multiple** roles | `[R1, R2, …]` | `--to-role R1 --discoverable` (grant R2… via SQL in step 3) |

```bash
cortex plugin publish <artifact_dir> --to-role <role> [--discoverable|--no-discoverable]
```

- **When to try:** shell is available and `<artifact_dir>` is readable. Skip in
  non-sandbox (SQL-only) runtime.
- **Surface once:** show [SKILL.md](SKILL.md) § **Cortex Extension — user-facing
  one-liner** if not shown yet.
- **On success — single role / PUBLIC:** share options are fully applied.
  Proceed to [step 5](step_5_report_result.md) — step 3 is not needed.
- **On success — multiple roles (choice 3):** the first role was granted by
  the CLI. Proceed to [step 3](step_3_apply_share_options.md) to grant the
  remaining roles via SQL (step 3 will skip the audience question since
  `<share_choice>` is already set).
- **Fall back to Option B** when the CLI is missing, the command exits
  non-zero, or the intent is `unshare` / `share-resync` / a rename pivot.
  Surface the CLI error verbatim before switching paths. The
  already-collected `<share_choice>` / `<share_roles>` / `<discoverable_value>`
  are passed to step 3 so the user is not asked again.
- **Stage limit error** — when the CLI error matches
  [SKILL.md](SKILL.md) § **Stage file size and count limits**, follow that
  section and **stop**; do **not** fall back to Option B.

## Option B: SQL fallback (full Cortex Extension pipeline)

Use this when Option A is unavailable or did not apply (see triggers above).
...
2. If `name` is missing, fall back to the artifact folder name only then:

| Path | Folder fallback |
| ---- | ---------------- |
| **Sandbox or CLI (client-local)** | `basename(<artifact_dir>)` |
| **Non-sandbox (SQL-only)** | Last segment of `<artifact_ws>` under `versions/live/` |
...
otherwise folder fallback from `<artifact_dir>` or `<artifact_ws>`). Compute
...
| Path | Timestamp |
| ---- | --------- |
| **Sandbox or CLI (client-local)** | `Date.now()` or equivalent in the client environment. |
| **Non-sandbox (SQL-only)** | `SELECT DATE_PART('epoch_millisecond', CURRENT_TIMESTAMP());` |

```sql
ALTER CORTEX EXTENSION <id> ADD LIVE VERSION <live_alias> FROM LAST;
```

If `ADD LIVE VERSION` fails with `099106` / "already a live version" /
...
fails for any **other** reason, surface the error and follow the
existing Commit or Abort section — do **not** loop `ABORT` /
`ADD LIVE VERSION`. On **Cancel sharing**, stop without aborting. Do
**not** silently ABORT.
...
### Fallback — `snow stage copy`

When `PUT` or `COPY FILES` fails, check for a **stage limit error** per
[SKILL.md](SKILL.md) § **Stage file size and count limits** — if matched,
follow that section and **stop** (do not load the fallback).

Otherwise **load and follow**
[step_2_fallback_stage_copy.md](step_2_fallback_stage_copy.md) (do not improvise a
different fallback). Destination URI patterns differ by `<artifact_type>` — see
that file.
...
**Build the “new artifact tree” file set:**

| Path | How |
| ---- | --- |
| **Sandbox or CLI (client-local)** | Same discovery as Path A upload (disk enumeration under `<artifact_dir>`). |
| **Non-sandbox (SQL-only)** | Same discovery as Path B upload (`LIST` on `<artifact_ws>`). |
...
If ANY of `ADD LIVE VERSION` / upload (`PUT`, `snow stage copy`, or `COPY FILES`)
/ `REMOVE` / `COMMIT` fails:
...
Route by intent, path (CLI vs SQL), and CLI outcome:

| Condition | Next step |
| --------- | --------- |
| `<intent> = share-resync` | [step 5](step_5_report_result.md) — content-only; no grant or DISCOVERABLE changes |
| Skill Option A CLI **success** | [step 5](step_5_report_result.md) — step 3 skipped |
| Plugin Option A CLI **success**, single/PUBLIC role | [step 5](step_5_report_result.md) — step 3 skipped |
| Plugin Option A CLI **success**, multi-role (choice 3) | [step 3](step_3_apply_share_options.md) — grant remaining roles; audience question skipped |
| SQL Option B (`share-first-time` or `share-resync-and-update-share-options`) | [step 3](step_3_apply_share_options.md) — apply share options via SQL |

When step 3 runs, it **skips its audience question** if step 1 or plugin
Option A already set `<share_choice>` / `<share_roles>` / `<discoverable_value>`.
