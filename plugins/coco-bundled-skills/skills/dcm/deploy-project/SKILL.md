...
### Stuck Deploy / Project Lock

Snowflake holds a server-side project lock for the entire duration of any `snow dcm deploy` execution. If the CLI times out, is killed client-side, or the deploy stalls for any reason, the Snowflake-side execution continues running and the lock remains held. Common causes include slow DDL operations, long-running Dynamic Table initializations, or transient Snowflake issues.

**Symptoms:**
- CLI hangs with the progress bar frozen at some percentage for 10-15 minutes with no movement
- Re-attempting deploy fails with: `Cannot execute DCM project because it is currently being executed by another process`

**Fix: find and cancel the stuck query**

**Step 1 - Discover the running deploy query:**

```sql
SELECT query_id, LEFT(query_text, 100) AS query_text, start_time
FROM TABLE(<DB>.INFORMATION_SCHEMA.QUERY_HISTORY_BY_USER(RESULT_LIMIT=>50))
WHERE execution_status = 'RUNNING'
  AND query_text ILIKE '%EXECUTE DCM PROJECT%'
  AND start_time >= DATEADD('hour', -1, CURRENT_TIMESTAMP())
ORDER BY start_time ASC;
```

- **If no rows are returned:** The server-side execution may have just completed before you ran the query. Wait 30 seconds and verify with `snow dcm plan`. If plan still fails with the lock error, contact Snowflake Support — do not attempt further cancellation steps.
- **If one row is returned:** Present the `query_id`, `query_text`, and `start_time` to the user and ask for explicit confirmation before proceeding to Step 2.
- **If multiple rows are returned:** Present all rows to the user and ask which `query_id` to cancel. If the user wants to cancel all of them, do so one at a time in `start_time ASC` order, requiring separate confirmation for each.

**Step 2 - Cancel the query (requires user confirmation):**

Show the user the query details from Step 1 and ask: "Found stuck query `<query_id>` started at `<start_time>`. Cancel it now?" Only proceed after the user confirms.

```sql
SELECT SYSTEM$CANCEL_QUERY('<query_id_from_above>');
```

After the query is cancelled, verify with `snow dcm plan`. If plan succeeds, the lock is gone — redeploy. If plan still returns the lock error, re-run the Step 1 discovery query. If no rows are returned at that point, contact Snowflake Support.

**Do NOT use `snow dcm drop-deployment`** to release the lock. It operates on deployment metadata only and may return a Snowflake internal error when the project is in a stuck state.
