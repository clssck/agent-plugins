...
Location: `${CLAUDE_PLUGIN_ROOT}/blueprints/*/overview.md`
...
for f in ${CLAUDE_PLUGIN_ROOT}/blueprints/*/overview.md; do echo "=== $f ==="; head -5 "$f"; done
```
...
Location: `${CLAUDE_PLUGIN_ROOT}/blueprints/*/step_*/overview.md`
...
grep -r -l -i "<topic>" ${CLAUDE_PLUGIN_ROOT}/blueprints/*/step_*/overview.md
```

### 3. Question Definitions (Practical Recommendations)
Location: `${CLAUDE_PLUGIN_ROOT}/definitions/questions.yaml`
...
1. **Search blueprint overviews** for the topic:
   ```bash
   grep -r -i "<topic_keywords>" ${CLAUDE_PLUGIN_ROOT}/blueprints/*/overview.md
   ```

2. **Search step overviews** for detailed guidance:
   ```bash
   grep -r -i "<topic_keywords>" ${CLAUDE_PLUGIN_ROOT}/blueprints/*/step_*/overview.md
   ```

3. **Search question definitions** for practical recommendations:
   ```bash
   grep -A 50 "<topic_keywords>" ${CLAUDE_PLUGIN_ROOT}/definitions/questions.yaml
   ```
...
grep -r -l -i "<topic>" ${CLAUDE_PLUGIN_ROOT}/blueprints/*/overview.md

# Search step overviews for detailed best practices
grep -r -l -i "<topic>" ${CLAUDE_PLUGIN_ROOT}/blueprints/*/step_*/overview.md

# Search question definitions for configuration recommendations
grep -A 50 -i "<topic>" ${CLAUDE_PLUGIN_ROOT}/definitions/questions.yaml
```
