---
description: "Show blueprint details including the task/step tree."
---
...
1. Load the blueprint's `meta.yaml` from `${CLAUDE_PLUGIN_ROOT}/blueprints/<blueprint-name>/meta.yaml`
...
1. Read `${CLAUDE_PLUGIN_ROOT}/blueprints/<blueprint-name>/meta.yaml`
2. Parse the YAML to get: `name`, `blueprint_id`, `summary`, `overview`, `is_repeatable`, `steps`
3. For each step in the `steps` list:
   - Check if `${CLAUDE_PLUGIN_ROOT}/blueprints/<blueprint-name>/<step-id>/` exists
