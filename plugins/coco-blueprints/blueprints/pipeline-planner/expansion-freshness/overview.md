...
How current does the new output from your expansion need to be? This determines how we configure refresh, scheduling, or triggers in the plan.

### Instructions

1. Ask: "How current does the expanded output need to be?"
2. Options: Continuous (within minutes) | Near Real-time (minutes to an hour) | Batch (hourly or longer)
3. Record the response as `data_freshness_requirement`.
