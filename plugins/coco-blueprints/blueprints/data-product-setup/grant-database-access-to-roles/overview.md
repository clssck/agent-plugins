This step grants database access to custom roles and any additional roles that need access to the data product. While the core role hierarchy was established in Step 3.3, this step focuses on:

1. **Granting Warehouse Access to Custom Roles** — Custom roles need warehouse access to execute queries
2. **Cross-Data Product Access** — Grant access to roles from other data products
3. **Service Account Access** — Configure access for application service accounts

This step is optional if you only use the core roles (READ, WRITE, CREATE, ADMIN).

**Account Context:** Execute this SQL in the target account using the RBAC role.

## Why is this important?

Proper database access grants ensure:
- **Query Execution**: Roles can actually run queries with warehouse access
- **Cross-Product Access**: Enable data sharing between data products
- **Service Integration**: Applications can access the data they need

## Prerequisites

- Custom read roles created (Step 4.1, if applicable)
- Warehouse access roles created (Step 3.2)

## Key Concepts

**Access Components**
A complete access grant requires both:
1. **Data Access** — Database role grants (SC_R, DB_R, etc.)
2. **Compute Access** — Warehouse usage grant

**Grant Flow**
```
User → Account Role → Warehouse Access Role → Warehouse
                 ↓
            Database Role → Schema → Objects
```

**Cross-Data Product Access**
To grant access to another data product's role:
```sql
