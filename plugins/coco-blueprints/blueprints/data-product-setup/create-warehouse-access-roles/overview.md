In this step, you'll create warehouse access roles that control who can use each warehouse. Following the principle of least privilege, access roles are separate from the warehouses themselves.

For each warehouse, a `_WH_U_<name>` (warehouse USAGE) role is created that grants:
- `USAGE` — Ability to use the warehouse for queries
- `MONITOR` — Ability to monitor warehouse activity
- `OPERATE` — Ability to start/stop the warehouse

**Account Context:** Execute this SQL in the target account using the ADMIN role.

## Why is this important?

Separate warehouse access roles enable:
- **Flexible Assignment**: Grant warehouse access independently of data access
- **Cost Control**: Restrict warehouse access to control compute spend
- **Audit Trail**: Clear visibility into who can use which warehouses
- **Least Privilege**: Users only get access to warehouses they need
...
| `MONITOR` | View warehouse metrics and query history |
| `OPERATE` | Resume, suspend, and abort queries |

**Role Ownership**
- Access roles are owned by the RBAC role
- RBAC role can grant these to other roles
